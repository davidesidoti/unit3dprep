...
|---|---|
| `U3DP_PASSWORD_HASH` | Hash bcrypt della password web. Generato da `generate_hash.py`. |
...
| Variabile | Default | Descrizione |
|---|---|---|
| `U3DP_HOST` | `127.0.0.1` | Bind address di uvicorn. Usa `0.0.0.0` su VPS con firewall pubblico, `127.0.0.1` dietro nginx reverse proxy. |
| `U3DP_PORT` | `8765` | Porta di ascolto. Su Ultra.cc usa una porta riservata (`app-ports free`). |
| `U3DP_ROOT_PATH` | `""` | Prefisso nginx (es. `/unit3dprep`). Vedi [note ROOT_PATH](#u3dp_root_path-e-nginx). |
| `U3DP_HTTPS_ONLY` | `0` | Se `1` imposta `https_only` sul cookie di sessione (richiesto dietro HTTPS). |
| `U3DP_MEDIA_ROOT` | `~/media` | Cartella base dei media. Le sottocartelle diventano categorie. |
| `U3DP_SEEDINGS_DIR` | `~/seedings` | Destinazione degli hardlink. Deve stare sullo stesso FS di `U3DP_MEDIA_ROOT`. |
| `U3DP_TMDB_LANG` | `it-IT` | Lingua delle risposte TMDB. Esempi: `en-US`, `es-ES`. |
...
| `U3DP_SYSTEMD_UNIT` | `unit3dprep.service` | Nome della systemd user unit usato dal bottone "Update app" per `systemctl --user cat/restart`. Su Ultra.cc impostare a `unit3dprep-web.service`. |
| `U3DP_GITHUB_REPO` | `davidesidoti/unit3dprep` | Slug `owner/repo` usato per il polling delle release (solo env, letto all'import). |
...
Le chiavi mascherate sono definite in `unit3dprep/web/config.py` (costante `MASKED_KEYS`):
...
- **App**: confronta la versione installata (`importlib.metadata.version("unit3dprep")`) con la [release più recente su GitHub](https://github.com/davidesidoti/unit3dprep/releases). Se `newer == true` compare il bottone "Update app".
