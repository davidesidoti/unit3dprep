...
| `U3DP_ROOT_PATH` | `""` | Nginx prefix (e.g. `/unit3dprep`). See [ROOT_PATH notes](#ita_root_path-and-nginx). |
...
- **App**: compares the installed version (`importlib.metadata.version("unit3dprep")`) against the [latest GitHub release](https://github.com/davidesidoti/unit3dprep/releases). If `newer == true` the "Update app" button appears.
