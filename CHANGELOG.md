...
- Il pannello "Possibile duplicato" del wizard mostra ora anche il **numero di file** del torrent
  trovato sul tracker.
- Nello step **Rinomina** del wizard, i file che si discostano dagli altri del pacchetto (es. un
  episodio H.264 in mezzo a H.265, o a 720p invece che 1080p) sono ora **evidenziati in rosso**
  con l'indicazione di cosa cambia rispetto alla maggioranza (risoluzione, codec, sorgente, HDR,
  audio, dub, gruppo), così è facile verificarli e correggere il nome prima di caricare.
...
- **Selezione multipla in Media Library**: ora si può selezionare **qualsiasi** elemento (film,
  ...
  **Shift+click** su una seconda card seleziona l'intero intervallo di media tra le due.
...
- Nel dettaglio di una serie, l'intestazione di una stagione mostra ora il badge giallo "da
  ...
  qualcosa da rivedere anche con l'accordion chiuso.
...
- La firma in coda alla descrizione dei torrent caricati sul tracker non è più "by Unit3DwebUp",
  ...
  (es. `Caricato con unit3dprep 1.1.5`). Viene sostituita automaticamente prima dell'upload.
